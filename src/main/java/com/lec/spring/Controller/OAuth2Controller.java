...
public class OAuth2Controller {
    ...
    @Autowired
    private UserService userService;   // oauth 로그인 회원가입 처리

    @Autowired
    private AuthenticationManager authenticationManager;   // 로그인처리 위해서 만듬
    ...
    public String kakaoCallBack(String code) {  // kakao 가 보내준 code 값 받아오기
        ...
        KakaoProfile profile = kakaoUserInfoUri(token.getAccess_token());
    ...
    public KakaoOAuthToken kakaoAccessToken(String code){
        ...
        ResponseEntity<String> response = rt.exchange(
               kakaoTokenUri,   // Access Token 요청
               HttpMethod.POST, // request method  -  요청지의 요청방법에 따라서
               kakaoTokenRequest,  // HttpEntity (body + header)
               String.class   // 응답받을 타임  바로받아도 됨 string 은 학습용
        );

        System.out.println("카카오 AccessToken 요청 응답: " + response);   // 확인용
        System.out.println("카카오 AccessToken 응답 body: " + response.getBody());   // 확인용
    ...
    // kakao 사용자 정보 요청하기
    private KakaoProfile kakaoUserInfoUri(String accessToken) {
        RestTemplate rt = new RestTemplate();

        // header 준비
        HttpHeaders headers = new HttpHeaders();
        headers.add("Authorization", "Bearer " + accessToken);  // Bearer 한칸 뛰기 잘맞출것
        headers.add("Content-type", "application/x-www-form-urlencoded;charset=utf-8");
    ...
    private User registerKakaoUser(KakaoProfile profile) {
     ...
     // 회원가입 진행하기 전에
     // 이미 기압힌 회원인지, 혹은 비가입자인지 체크하여야 한다.
     User user = userService.findByUsername(username);
        if (user == null) {   // 미가입자인 경우 회원가입 진행
            ...
            if(cnt > 0) {
                ...
                user = userService.findByUsername(username);  // 다시 읽어오기, regDate 정도
            }else {
    ...
    // 로그인 진행  시큐리티에서 로그인상태로 잡아주도록 하는 로직
