...
public class AttachmentController {
    ...
    public ResponseEntity<Object> download(Long id){
        if(id == null) return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);  // 400 에러 띄우기

        Attachment file = attachmentService.findById(id);
        if(file == null) return new ResponseEntity<>(null, HttpStatus.NOT_FOUND);  // 404 에러 띄우기
        ...
        try {
           ...
           // 유형이 지정되지 않은 경우
            ...
            return new ResponseEntity<>(resource, headers, HttpStatus.OK); // 200 성공!
